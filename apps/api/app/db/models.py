...
class AgentProposal(TenantModel):
    """An observation-based action proposal generated by the autonomous agent loop."""
    ...
    entity_ids: Mapped[list] = mapped_column(JSONB, default=list, server_default="[]")
    context_snapshot: Mapped[dict] = mapped_column(JSONB, default=dict, server_default="{}")
    proposed_for_user_id: Mapped[uuid.UUID | None] = mapped_column(
...
class AgentConfig(TenantModel):
    """Per-org configuration for the autonomous agent: thresholds, personality, learning state."""
    ...
    auto_approve_threshold: Mapped[float] = mapped_column(Float, default=0.95)
    patterns_enabled: Mapped[list] = mapped_column(
        JSONB, default=list,
